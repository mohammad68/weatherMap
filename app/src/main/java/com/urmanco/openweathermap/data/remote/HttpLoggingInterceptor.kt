...
class HttpLoggingInterceptor @JvmOverloads constructor(private val logger: Logger = Logger.DEFAULT) :
        ...
        companion object {
            ...
            val DEFAULT: Logger = object : Logger {
                override fun log(message: String?) {
                   Log.i(Log.INFO.toString(), message, null)
                }
            }
        }
    }
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        val level = level
        val request = chain.request().newBuilder().addHeader("platform", "kidide").build()
        ...
        if (logHeaders) {
            ...
            if (!logBody || !hasRequestBody) {
                ...
                var charset = Charset.forName("UTF8") // UTF8;
                ...
                if (isPlaintext(buffer)) {
                    ...
                    val strings = string.split("\\r?\\n").toTypedArray()
                    for (subStr: String? in strings) {
                        logger.log(subStr)
                    }
                    logger.log(
                        "--> END " + request.method
